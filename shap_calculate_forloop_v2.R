...
ncore<-as.numeric(args[7])         # number of cores reserved
...
library(grf)
library(future)
devtools::load_all("shapr-master",recompile=FALSE)
...
if ((start+MC-1)<=2480){
  ...
  for(ii in start:(start+MC-1)){
    ...
    out_list[[k]]<-explanation
    saveRDS(out_list,paste0(proj.dir,"output/output_tr",tr,"_START",start,"_END",start+MC-1,"_NC",nc,"_NS",nc,"_NB",nb))
    k<-k+1
  }

} else if (start<2481 & (start+MC-1)>2480) {

  for(ii in start:2480){
    ...
    out_list[[k]]<-explanation
    saveRDS(out_list,paste0(proj.dir,"output/output_tr",tr,"_START",start,"_END",2480,"_NC",nc,"_NS",nc,"_NB",nb))
    k<-k+1
